...
public class SortMassive {
    ...
    public static void main(String[] args) throws IOException {
        System.out.println("Сколько числе будет в массиве:");
    ...
    private static int[] mergeMass(int[] sortL, int[] sortR) {
        int[] resultMasInMerge = new int[sortL.length + sortR.length];
        int i = 0, j = 0, resI = 0;
        while (i < sortL.length && j < sortR.length) {
            if (sortL[i] < sortR[j]) {
                resultMasInMerge[resI] = sortL[i];
                i++;
                resI++;
                if (i == sortL.length) {
                    for (int k = j; k < sortR.length; k++, resI++) {
                        resultMasInMerge[resI] = sortR[k];
                    }
                }
            } else {
                resultMasInMerge[resI] = sortR[j];
                j++;
                resI++;

                if (j == sortR.length) {
                    for (int k = i; k < sortL.length; ) {
                        resultMasInMerge[resI] = sortL[k];
                        k++;
                        resI++;
                    }
                }
